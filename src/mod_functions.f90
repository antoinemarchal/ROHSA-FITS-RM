...
module mod_functions
  ...
  public :: mean_array, mean_map, dim2nside, dim_data2dim_cube, reshape_up, reshape_down, go_up_level, init_spectrum, &
       upgrade, update, set_stdmap, std_spectrum, mean_spectrum, max_spectrum, init_grid_params, reshape_noise_up, &
       sum_map_square
...
contains
  ...
  subroutine sum_map_square(nside, map, map_mean)
    !! Average cube along spatial axis depending on level n 
    implicit none

    integer, intent(in) :: nside !! nside of the cube
    real(xp), intent(in), dimension(:,:), allocatable :: map !! cube
    real(xp), intent(inout), dimension(:,:), allocatable :: map_mean !! average cube
    ...
    do i=1,size(map_mean,dim=1)
       do j=1,size(map_mean,dim=2)
          do k=1,n
             do l=1,n
                val = val + map(k+((i-1)*n),l+((j-1)*n))**2._xp
             enddo
          enddo
          map_mean(i,j) = val
          val = 0.
